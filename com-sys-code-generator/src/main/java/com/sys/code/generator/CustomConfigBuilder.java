...
public class CustomConfigBuilder extends ConfigBuilder {
     ...
     */
    public CustomConfigBuilder(PackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig, TemplateConfig template, GlobalConfig globalConfig) {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.ENTITY, joinPackage(config, config.getEntity()));
        packageInfo.put(ConstVal.MAPPER, joinPackage(config, config.getMapper()));
        packageInfo.put(ConstVal.XML, joinPackage(config, config.getXml()));
        packageInfo.put(ConstVal.SERVICE, joinPackage(config, config.getService()));
        packageInfo.put(ConstVal.SERVICE_IMPL, joinPackage(config, config.getServiceImpl()));
        packageInfo.put(ConstVal.CONTROLLER, joinPackage(config, config.getController()));
     ...
     * @return 连接后的包名
     */
    private String joinPackage(PackageConfig config, String subPackage) {
        return subPackage;
    }
...
//            pathInfo.put(path, joinPath(outputDir, packageInfo.get(module)));
    ...
    public CustomConfigBuilder setTableInfoList(List<TableInfo> tableInfoList) {
